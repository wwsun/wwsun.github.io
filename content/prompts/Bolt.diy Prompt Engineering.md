---
title: Bolt.diy Prompt Engineering
tags:
draft: false
description: 未命名
...
<mobile_app_instructions>
  ...
  4. Prefer built-in React Native components for core UI elements:
     - View, Text, TextInput, ScrollView, FlatList, etc.
     - Image for displaying images
     - TouchableOpacity or Pressable for press interactions

  5. For advanced components, use libraries compatible with Expo:
     - React Native Paper
     - Native Base
  ...
  7. Use memo and useCallback for expensive components/functions
  8. Implement virtualized lists (FlatList, SectionList) for large data sets
  9. Use appropriate image sizes and formats
  10. Implement proper list item key patterns
  11. Minimize JS thread blocking operations

  ACCESSIBILITY:

  12. Use appropriate accessibility props:
     - accessibilityLabel
     - accessibilityHint
     - accessibilityRole
  13. Ensure touch targets are at least 44×44 points
  14. Test with screen readers (VoiceOver on iOS, TalkBack on Android)
  15. Support Dark Mode with appropriate color schemes
  16. Implement reduced motion alternatives for animations

  DESIGN PATTERNS:

  17. Follow platform-specific design guidelines:
  ...
  21. For Metro bundler issues:
     - Clear cache with \`npx expo start -c\`
     - Check for dependency conflicts
     - Verify Node.js version compatibility

  22. For TypeScript errors:
     - Ensure proper typing
     - Update tsconfig.json as needed
     - Use type assertions sparingly

  23. For native module issues:
...
- **单一真理源**：规定每次回复只生成**一个** `<boltArtifact>`，包含所有必要的文件和命令。
- **全量更新**：严禁使用 diff 或 `// ... rest of code` 占位符。强制 AI 每次修改文件时输出完整内容，确保代码的一致性（虽然费 Token，但最稳健）。
...
- **UI 细节**：对排版、间距（8pt grid）、微交互（Hover states）都做了具体要求。

### 移动端开发 `<mobile_app_instructions>`

- **Expo 独占**：明确在 WebContainer 中开发移动应用只能使用 **Expo** 和 **React Native**。
- **库限制**：推荐使用 `React Navigation` 和 `Gluestack UI` (或类似库)，避免 AI 推荐不兼容的原生模块。
