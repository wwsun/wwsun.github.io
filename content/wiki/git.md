...
`--no-ff`（no fast-forward）参数用于在合并分支时禁用“快进合并”（Fast-forward merge），从而强制生成一个新的合并提交（Merge Commit）。

| 特性                 | 默认合并（快进合并，条件允许时）       | 强制非快进合并（`--no-ff`）            |
| -------------------- | -------------------------------------- | -------------------------------------- |
| **是否生成新提交**   | 否                                     | 是                                     |
| **提交历史结构**     | 呈现为一条直线                         | 呈现出分支分叉与合并的拓扑图谱         |
| **回滚功能的难易度** | 较难，需要处理多个提交或重置指针       | 极易，只需回滚单个合并提交             |
| **适用场景**         | ==个人本地开发，或者同步上游最新更新== | ==团队协作==，将独立特性分支合并回主干 |
