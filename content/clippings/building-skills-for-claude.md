...
**技能（Skill）** 是一组指令——打包为一个简单的文件夹——教 Claude 如何处理特定任务或工作流。技能是满足特定需求定制 Claude 的最强大方式之一。无需在每次对话中反复解释你的偏好、流程和领域专业知识，**技能**让你教 Claude 一次，每次都能受益。
...
💡 **构建不涉及 MCP 的独立技能？** 跳到规划与设计——你可以随时回来看这部分。
...
- **在 X 次工具调用内完成工作流**
  - _如何衡量：_ 比较启用和未启用技能时相同任务的差异。统计工具调用次数和总 token 消耗量。
- **每次工作流零失败 API 调用**
...
**skill-creator** 技能——可通过 Claude.ai 插件目录或下载用于 Claude Code——可以帮助你构建和迭代技能。
...
> **注意：** skill-creator 帮助你设计和优化技能，但不执行自动化测试套件或产生量化评估结果。
...
> **注意 README.md 的位置：** README.md 放在仓库**根目录**供人类参考，但**不在技能文件夹内部**——这与技能文件夹不能包含 README.md 的要求不冲突。
...
description: 跨 PagerDuty、Slack 和 Jira 协调事件响应。当用户提到"incident"、"outage"、"page"或"SEV-1"时使用。
---
```
...
> **译者注：** 本文翻译自 Anthropic 官方发布的《The Complete Guide to Building Skills for Claude》PDF。翻译力求准确传达原文意思，同时保持中文表达的流畅性。如有翻译不当之处，欢迎指正。
