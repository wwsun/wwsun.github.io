---
title: Developing an Agent Framework from 0 to 1 | BestBlogs.dev
source: https://www.bestblogs.dev/en/article/bc280b72
author:
  - "[[Datawhale]]"
...
本节重点给大家介绍我们的升级方向，代码不会展开。代码实践查看： `[https://datawhalechina.github.io/hello-agents，我们将把这个基础客户端，改造为一个更具适应性的模型调用中枢。本次升级主要围绕以下三个目标展开：](https://datawhalechina.github.io/hello-agents%EF%BC%8C%E6%88%91%E4%BB%AC%E5%B0%86%E6%8A%8A%E8%BF%99%E4%B8%AA%E5%9F%BA%E7%A1%80%E5%AE%A2%E6%88%B7%E7%AB%AF%EF%BC%8C%E6%94%B9%E9%80%A0%E4%B8%BA%E4%B8%80%E4%B8%AA%E6%9B%B4%E5%85%B7%E9%80%82%E5%BA%94%E6%80%A7%E7%9A%84%E6%A8%A1%E5%9E%8B%E8%B0%83%E7%94%A8%E4%B8%AD%E6%9E%A2%E3%80%82%E6%9C%AC%E6%AC%A1%E5%8D%87%E7%BA%A7%E4%B8%BB%E8%A6%81%E5%9B%B4%E7%BB%95%E4%BB%A5%E4%B8%8B%E4%B8%89%E4%B8%AA%E7%9B%AE%E6%A0%87%E5%B1%95%E5%BC%80%EF%BC%9A)`
...
安装完成后，使用以下命令即可启动一个兼容 OpenAI 的 API 服务。VLLM 会自动从 Hugging Face Hub 下载指定的模型权重（如果本地不存在）。服务启动后，便会在 `[http://localhost:8000/v1](http://localhost:8000/v1)` 地址上提供与 OpenAI 兼容的 API。
...
当你在终端看到模型的交互提示时，即表示服务已经成功在后台启动。Ollama 默认会在 `[http://localhost:11434/v1](http://localhost:11434/v1)` 地址上暴露 OpenAI 兼容的 API 接口。
...
4. **辅助判断：分析 API 密钥的格式** 在某些情况下，如果上述两种方式都无法确定，框架会尝试分析通用环境变量 `LLM_API_KEY` 的格式。例如，某些服务商的 API 密钥有固定的前缀或独特的编码格式。不过，由于这种方式可能存在模糊性（例如多个服务商的密钥格式相似），因此它的优先级较低，仅作为辅助手段。
...
![](https://wechat2rss.bestblogs.dev/img-proxy/?k=34eaf399&u=https%3A%2F%2Fmmbiz.qpic.cn%2Fsz_mmbiz_png%2FvI9nYe94fsFHo3rmJswuRAbqHWyUysG9h7wRNMfpQHG0qyT6rgiaVAMUKFFdF0qCCVYwicNeEibRSBqdRvYdsmGVA%2F640%3Fwx_fmt%3Dpng%26from%3Dappmsg)
...
def__init__(self, content: str, role: MessageRole, **kwargs):
        super().__init__(
            content=content,
            role=role,
...
def__init__(
    ...
    @abstractmethod
defrun(self, input_text: str, **kwargs) -> str:
"""运行Agent"""
pass
...
"""清空历史记录"""
        self._history.clear()
defget_history(self) -> list[Message]:
"""获取历史记录"""
...
classMySimpleAgent(SimpleAgent):
"""
    重写的简单对话Agent
    ...
    """def__init__(
        ...
        self.enable_tool_calling = enable_tool_calling and tool_registry isnotNone
...
classMySimpleAgent(SimpleAgent):
# ... 前面的 __init__ 方法
defrun(self, input_text: str, max_tool_iterations: int = 3, **kwargs) -> str:
"""
...
def_get_enhanced_system_prompt(self) -> str:
"""构建增强的系统提示词，包含工具信息"""
...
classMySimpleAgent(SimpleAgent):
# ... 前面的方法
def_run_with_tools(self, messages: list, input_text: str, max_tool_iterations: int, **kwargs) -> str:
"""支持工具调用的运行逻辑"""
        current_iteration = 0
        final_response = ""
...
def_parse_tool_calls(self, text: str) -> list:
"""解析文本中的工具调用"""
        pattern = r'\[TOOL_CALL:([^:]+):([^\]]+)\]'
...
return tool_calls
def_execute_tool_call(self, tool_name: str, parameters: str) -> str:
"""执行工具调用"""
...
def_parse_tool_parameters(self, tool_name: str, parameters: str) -> dict:
"""智能解析工具参数"""
        param_dict = {}
if'='in parameters:
...
classMySimpleAgent(SimpleAgent):
# ... 前面的方法
defstream_run(self, input_text: str, **kwargs) -> Iterator[str]:
"""
        自定义的流式运行方法
...
defhas_tools(self) -> bool:
"""检查是否有可用工具"""
return self.enable_tool_calling and self.tool_registry isnotNone
defremove_tool(self, tool_name: str) -> bool:
"""移除工具（便利方法）"""
if self.tool_registry:
            self.tool_registry.unregister(tool_name)
returnTrue
returnFalse
deflist_tools(self) -> list:
"""列出所有可用工具"""
if self.tool_registry:
...
由于这几类Agent已经实现过核心逻辑，所以这里只给出对应的Prompt。与之前专门针对代码生成的提示词不同，框架化的版本采用了通用化设计，使其适用于文本生成、分析、创作等多种场景，并通过 `custom_prompts` 参数支持用户深度定制。
...
![](https://wechat2rss.bestblogs.dev/img-proxy/?k=6df68574&u=https%3A%2F%2Fmmbiz.qpic.cn%2Fsz_mmbiz_png%2FvI9nYe94fsFHo3rmJswuRAbqHWyUysG9BNicwKx7BulOYft2ANlEj4kgeSXuqNcNSFQk91boehduuU4mTELaib1w%2F640%3Fwx_fmt%3Dpng%26from%3Dappmsg)
...
```
def_invoke_with_tools(self, messages: list[dict[str, Any]], tools: list[dict[str, Any]], tool_choice: Union[str, dict], **kwargs):
"""调用底层OpenAI客户端执行函数调用"""
...
def__init__(self, name: str, description: str):
    ...
    @abstractmethod
defrun(self, parameters: Dict[str, Any]) -> str:
"""执行工具"""
pass
    @abstractmethod
defget_parameters(self) -> List[ToolParameter]:
"""获取工具参数定义"""
pass
```
...
defregister_tool(self, tool: Tool):
"""注册Tool对象"""
if tool.name in self._tools:
            print(f"⚠️ 警告:工具 '{tool.name}' 已存在，将被覆盖。")
        self._tools[tool.name] = tool
        print(f"✅ 工具 '{tool.name}' 已注册。")
defregister_function(self, name: str, description: str, func: Callable[[str], str]):
"""
        直接注册函数作为工具（简便方式）
        Args:
            name: 工具名称
...
```
defget_tools_description(self) -> str:
"""获取所有可用工具的格式化描述字符串"""
    descriptions = []
# Tool对象描述
...
```
defto_openai_schema(self) -> Dict[str, Any]:
...
import math
from hello_agents import ToolRegistry
defmy_calculate(expression: str) -> str:
"""简单的数学计算函数"""
...
def_eval_node(node, operators, functions):
"""简化的表达式求值"""
...
![](https://wechat2rss.bestblogs.dev/img-proxy/?k=df4af110&u=https%3A%2F%2Fmmbiz.qpic.cn%2Fsz_mmbiz_png%2FvI9nYe94fsFHo3rmJswuRAbqHWyUysG97T3FBuDdQQ155kqv7icqoaqc2Jtk2ZkHYWeecGWUhLlFmGDkbdJQFww%2F640%3Fwx_fmt%3Dpng%26from%3Dappmsg)
...
"""
    ...
    """def__init__(self, backend: str = "hybrid", tavily_key: Optional[str] = None, serpapi_key: Optional[str] = None):
        super().__init__(
            name="search",
...
```
def_search_hybrid(self, query: str) -> str:
"""混合搜索 - 智能选择最佳搜索源"""
# 优先使用Tavily（AI优化的搜索）
...
```
def_search_tavily(self, query: str) -> str:
"""使用Tavily搜索"""
...
配置后重新运行程序。"""        print(f"🔍 开始智能搜索: {query}")
# 尝试多个搜索源，返回最佳结果
...
def_search_with_tavily(self, query: str) -> str:
"""使用Tavily搜索"""
...
return result
def_search_with_serpapi(self, query: str) -> str:
"""使用SerpApi搜索"""
import serpapi
...
return result
defcreate_advanced_search_registry():
"""创建包含高级搜索工具的注册表"""
    registry = ToolRegistry()
# 创建搜索工具实例
...
defadd_step(self, tool_name: str, input_template: str, output_key: str = None):
"""
        添加工具执行步骤
        Args:
            tool_name: 工具名称
...
defexecute(self, registry: ToolRegistry, initial_input: str, context: Dict[str, Any] = None) -> str:
"""执行工具链"""
        context = context or {}
...
return final_result
classToolChainManager:
"""工具链管理器"""
def__init__(self, registry: ToolRegistry):
        self.registry = registry
        self.chains: Dict[str, ToolChain] = {}
defregister_chain(self, chain: ToolChain):
"""注册工具链"""
        self.chains[chain.name] = chain
        print(f"✅ 工具链 '{chain.name}' 已注册")
defexecute_chain(self, chain_name: str, input_data: str, context: Dict[str, Any] = None) -> str:
"""执行指定的工具链"""
if chain_name notin self.chains:
returnf"❌ 工具链 '{chain_name}' 不存在"
        chain = self.chains[chain_name]
return chain.execute(self.registry, input_data, context)
deflist_chains(self) -> List[str]:
"""列出所有工具链"""
return list(self.chains.keys())
# 使用示例
defcreate_research_chain() -> ToolChain:
"""创建一个研究工具链:搜索 -> 计算 -> 总结"""
    chain = ToolChain(
...
from hello_agents import ToolRegistry
classAsyncToolExecutor:
"""异步工具执行器"""
def__init__(self, registry: ToolRegistry, max_workers: int = 4):
        self.registry = registry
        self.executor = concurrent.futures.ThreadPoolExecutor(max_workers=max_workers)
asyncdefexecute_tool_async(self, tool_name: str, input_data: str) -> str:
"""异步执行单个工具"""
...
return result
asyncdefexecute_tools_parallel(self, tasks: List[Dict[str, str]]) -> List[str]:
"""并行执行多个工具"""
...
# 使用示例
asyncdeftest_parallel_execution():
"""测试并行工具执行"""
from hello_agents import ToolRegistry
    registry = ToolRegistry()
# 假设已经注册了搜索和计算工具
