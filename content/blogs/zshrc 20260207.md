...
draft: false
description: >-
  .zshrc - Zsh 配置文件 # 环境: macOS (Intel) + Ghostty + Zsh
source: null
date: '2026-02-07'
---
...
ssh_agent_start() {
  ...
  # 检查当前 agent 状态 (0: 有密钥, 1: 无密钥可用, 2: 无法连通)
  ...
  # 如果连不上 agent（如未启动），尝试加载缓存环境或启动新 agent
  ...
  # 批量加载 macOS keychain 中的密钥（兼容 Apple 的特有指令）
  if [[ "$OSTYPE" == "darwin"* ]]; then
    ssh-add --apple-load-keychain 2>/dev/null || ssh-add -A 2>/dev/null
  fi

  # 添加常用密钥（含 id_netease 等，若已在 keychain 中则会静默成功）
